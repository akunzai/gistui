...
- **View-model seam (issues #241 / #250):** each frame builds a pure `ViewModel` (`ChromeVm` + `ScreenVm`) at the draw entry. Migrated bodies (List, Pins, Confirm modal, Help) paint from the VM only; remaining screens are `ScreenVm::Legacy` and may still read `AppState` for body paint. Do **not** put business rules or FS/network work in paint helpers.
