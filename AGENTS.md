...
- `tui` is a screen state machine (`Screen::{List, Diff, Confirm, Preview, Help(payload), Pins, Gists, …}`; `Gists` is the gist-level manager). **Screen is `Clone`, not `Copy`** — `Help` carries `Box<HelpState>` so help topic/scroll/return live on the active screen (issue #242; other screens still use unit variants + parallel fields for now). `AppState::handle_key` is **pure** — it mutates state and returns a `KeyOutcome` intent; `run_loop` performs the IO (fetch/download/upload/create/delete/remove-file/edit-description). Keep new key logic in `handle_key` (testable) and new IO in `run_loop` helpers.
