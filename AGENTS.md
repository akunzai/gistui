...
- `tui` is a screen state machine (`Screen::{List, Diff, Confirm, Preview, Help(payload), Config(payload), Revisions(payload), Pins(payload), Gists(payload), GistDetail(payload), …}`; `Gists` is the gist-level manager). **Screen is `Clone`, not `Copy`** — payload variants hold screen-local state (issue #242; remaining screens still use unit tags + parallel fields). Revision/pin diffs park their payload on `diff_return`; detail parks on `preview_return` / compact restore; opening detail from Gists parks the Gists payload on `detail.return_screen` so Esc restores list state. `AppState::handle_key` is **pure** — it mutates state and returns a `KeyOutcome` intent; `run_loop` performs the IO (fetch/download/upload/create/delete/remove-file/edit-description). Keep new key logic in `handle_key` (testable) and new IO in `run_loop` helpers.
