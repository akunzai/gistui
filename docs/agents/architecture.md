...
- A new preference is: a `Preferences` field (+ its default), a `config_fields!` line (label, Settings description, `?` help — `ConfigField::ALL` and the Config help topic are generated from it), and its arms in `adjust` / `field_value` (exhaustive; on/off fields go through `flag_mut`). Saving needs nothing. `save_config` edits the existing file with `toml_edit` (#484): only keys whose value this save changes are written (or removed when back at their default), comments and unknown keys stay, `[[pinned]]` tables are matched by their key and updated in place. A missing file is written fresh with every non-default key in declaration order (`toml` `preserve_order`), plus `pinned`.
