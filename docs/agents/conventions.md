...
- Creating a version milestone (`gh api repos/{owner}/{repo}/milestones -f title=... -f state=open`) sets `-f description="<one-line summary>"` in the same call — the field has no default and nothing else prompts for it.
